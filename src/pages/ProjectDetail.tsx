...
export default function ProjectDetail() {
  ...
  const markStagesAsViewed = async (stages: any[]) => {
    try {
      // Mark stages as viewed if they have been approved OR payment received
      // This clears both "Approved" and "Payment Received" notifications
      const stagesToMarkViewed = stages
        .filter(s => 
          (s.approved_at && !s.viewed_by_freelancer_at) || 
          (s.payment_status === 'received' && !s.viewed_by_freelancer_at)
        )
        .map(s => s.id);
      
      if (stagesToMarkViewed.length > 0) {
        await supabase
          .from('stages')
          .update({ viewed_by_freelancer_at: new Date().toISOString() })
          .in('id', stagesToMarkViewed);
        console.log(`✅ Marked ${stagesToMarkViewed.length} stages as viewed`);
      }
    ...
    try {
      ...
      try {
        ...
        if (project && project.client_email) {
          ...
          console.log('[Mark as Delivered] ✅ Stage delivered email sent to client');
        }
      } catch (emailError: any) {
        console.error('[Mark as Delivered] Email sending failed (non-critical):', emailError.message);
        // Don't throw - delivery marking still succeeds even if email fails
      }
...
<div className="mb-8">
  ...
  <div className="flex sm:hidden flex-col gap-4">
    <div className="flex items-start justify-between gap-3">
      <h1 className="text-2xl font-black tracking-tight text-black flex-1 min-w-0">
      ...
      <button
        ...
        className="px-3 py-2 border-2 border-gray-200 hover:border-gray-300 bg-white text-gray-700 rounded-lg transition-all flex items-center justify-center flex-shrink-0"
        title="Refresh"
      >
        <svg 
            ...
            >
              ...
              <div className="p-6 sm:p-8 space-y-8 sm:space-y-12">
                ...
                <div className="bg-gray-50 border-2 border-gray-200 rounded-xl p-6">
                  ...
                  {stage.status === 'delivered' && stage.revisions_used === 0 && (
                    <div className="mt-6 flex justify-center">
                      <div className="px-8 py-3 bg-green-100 border-2 border-green-500 text-green-700 rounded-lg font-semibold">
                        ✓ Submitted
                      </div>
                    </div>
                  )}
